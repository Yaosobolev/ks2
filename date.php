...
include "connect.php";
  ...
  mysql_select_db("Fortest");
    ...
    echo "<form method=\"POST\" action=\"date.php\">\n";
...
WHERE  TestingDate = \"$_GET[date_id]\"  and Groups.id= TestingResult.GroupID
ORDER BY Surname ASC";  //"$_GET[date_id]\" Groups.id = TestingResult.GroupID AND
	...
	{	
	  ...
	  if(isset($_GET["print"]))
	echo "<H3 style=\"margin-left: 5em;\"> Отчет o результатах тестирования по выбранной дате.</H3>";
	else	  
	  echo "<br><H4 align=left>Результаты тестирования по выбранной дате.</H4>";
	...
	{  
		...
		echo "<a style=\"float: right;margin-right: 10px;\" href=\"date.php?date_id=$_GET[date_id]&print=1\" target=blank> Версия для печати </a>";	
